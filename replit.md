...
-   **Authentication & Authorization**: Replit Auth (OIDC PKCE) with role-based access control (admin/user roles). `requireAuth` and `requireAdmin` middleware for protected routes. Admin user management endpoints (`GET /api/auth/users`, `PUT /api/auth/users/:id/role`). First user should be promoted to admin via SQL.
