...
-   **Core Features**:
    ...
    -   **Coding Workbench**: Multi-panel IDE-like workbench (`/workbench`) with AI code chat, shell terminal, SSH remote terminal (connect to VPS via password or private key, run commands, SFTP file transfer), file explorer, Git panel, database explorer, environment variables viewer, and Catppuccin Mocha dark theme. All panel state persisted to localStorage (chat messages, shell history, panel layout, file selections).
    -   **Claude Workbench**: Advanced IDE-like workbench (`/claude-workbench`) with 13 configurable panels in Catppuccin Mocha dark theme: Claude Code chat (Sonnet), AI Router (auto/manual model selection across Sonnet/Opus/Haiku), Shell, File Explorer, Preview (iframe), Git, Agent Activity, Database, Security Scanner, Skills Browser, Code Review (AI-powered with grades), Env Vars, and Process Info. All panel state persisted to localStorage.
