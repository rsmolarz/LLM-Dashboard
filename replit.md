...
-   **Core Features**:
    ...
    -   **LLM Manager**: Dedicated model management page at `/llm-manager` route. Stats cards (total models, loaded, disk usage, server URL), active-in-memory models section (RAM/VRAM/TTL), full model list (name, size, params, family, quantization, digest), pull and delete actions, auto-refresh. Uses existing `useListModels`/`useListRunningModels`/`useGetLlmStatus`/`useGetLlmConfig` hooks. Sidebar entry in Admin section.
    -   **Code Agent**: Full Claude Code-like AI coding assistant at `/code` route. Features: (1) AI chat panel with streaming responses from any model (VPS Ollama or OpenRouter), (2) File explorer with workspace directory browsing, (3) Tabbed code editor with save/edit functionality, (4) Integrated terminal for command execution, (5) Smart model selector dropdown showing VPS models (free) and OpenRouter models (paid) with pricing info, (6) Code block actions — "Apply" writes file paths directly to editor tabs, "Run" executes bash commands in terminal, (7) Open file context automatically injected into AI prompts, (8) Layout modes: agent-only, editor-only, or split view, (9) GitHub/GitLab/Bitbucket repo cloning via Sources panel (shallow clone into projects/ folder, re-clone pulls latest), (10) Workspace project browser showing all artifacts, libraries, and cloned repos. Backend: `code-terminal.ts` with path traversal protection (workspace-root enforcement), unified `/code-terminal/chat` endpoint routing to Ollama or OpenRouter based on model name, file CRUD with sanitized paths. New endpoints: `POST /api/code-terminal/clone-repo` (clone git repos), `GET /api/code-terminal/projects` (list workspace projects).
    -   **Chat Import System**: Import conversations from ChatGPT, Claude, Gemini, and plain text/Markdown files. Route: `/chat-import`. Features: drag-and-drop file upload, auto-format detection (ChatGPT JSON, Claude JSON, Gemini JSON, Markdown/Text), conversation preview before import, browsable imported history with message viewer, delete imported conversations, import stats. Backend: `chat-import.ts` with routes: `POST /api/chat-import/parse` (detect format & preview), `POST /api/chat-import/import` (store to DB), `GET /api/chat-import/imported` (list imported), `GET /api/chat-import/imported/:id/messages` (view messages), `DELETE /api/chat-import/imported/:id` (delete), `GET /api/chat-import/stats` (counts by source). Imported conversations stored in `conversations`/`chat_messages` tables with systemPrompt marker `"Imported from {source} via {filename}"`.
    -   **Claw Code Agent**: Full integration of the Claw Code Agent Python framework at `/claw-agent`. Three tabs: (1) Overview — architecture description, key features, Ollama integration details, and VPS quick-start commands; (2) Source Browser — interactive file tree with syntax-highlighted code viewer for all 54 Python source files; (3) Configuration — model/base-URL/temperature/permissions configurator that generates ready-to-run CLI commands. Backend: `claw-agent.ts` with `GET /api/claw-agent/tree` (recursive file tree), `GET /api/claw-agent/file?path=` (file content), `GET /api/claw-agent/info` (project metadata from pyproject.toml). Source stored in `claw-code-agent/` directory.
