...
-   **Core Features**:
    ...
    -   **Coding Workbench**: Multi-panel IDE-like workbench (`/workbench`) with AI code chat, shell terminal, SSH remote terminal (connect to VPS via password or private key, run commands, SFTP file transfer, AI chat with model selector dropdown), file explorer (inline create/rename/delete plus file uploads via toolbar Upload button and desktop drag-drop, all routed through the per-user scratch dir with quota enforcement), Git panel, database explorer, environment variables viewer, and Catppuccin Mocha dark theme. SSH AI chat supports multi-provider model selection (OpenRouter, Anthropic, Ollama) with tool calling for remote server management. All panel state persisted to localStorage.
    -   **Project Sidebar**: Shared sidebar (`ProjectSidebar.tsx`) across all three workbenches showing 83+ projects from 3 sources: Replit registry (71 apps from `data/replit-projects.json`), VPS (SSH-discovered), and Local (workspace). Features search, source filtering (All/Replit/VPS/Local), color-coded initials, visibility badges (public/private), and click-to-open Replit apps in new tab.
