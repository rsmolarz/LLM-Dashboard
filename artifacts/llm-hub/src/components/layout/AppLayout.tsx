...
import {
  Server, MessageSquare, Brain, Terminal, Bot, Search, Eye, BarChart3, LineChart,
  Zap, Menu, X, Shield, LogIn, LogOut, User, Stethoscope, Share2, TrendingUp,
...
const navSections: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/research-pipeline", label: "Pipeline", icon: FlaskConical, adminOnly: false },
    ],
  },
  {
    title: "Knowledge",
    items: [
      ...
      { href: "/pipeline", label: "Data Pipeline", icon: Beaker, adminOnly: false },
...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>(() => {
    ...
    navSections.forEach(s => { defaults[s.title] = true; });
    return defaults;
  });
  ...
  return (
    <div className="min-h-screen h-screen bg-background text-foreground flex font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden fixed inset-0 top-14 z-30 bg-black/80 backdrop-blur-sm" onClick={() => setMobileMenuOpen(false)}>
            <nav className="bg-background border-r border-white/5 w-64 h-full overflow-y-auto py-4" onClick={(e) => e.stopPropagation()}>
              {filteredSections.map((section) => (
