...
export default function PubMedCollector() {
  ...
  const fetchArticles = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/pubmed-ent/articles`);
      if (res.ok) {
      ...
      {activeTab === "articles" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">{articles.length} articles in cache</p>
            <button
              onClick={fetchArticles}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-muted-foreground hover:text-white hover:bg-white/5 transition-all"
            >
          ...
          {articles.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground text-xs mt-1">Run a collection to fetch PubMed articles</p>
            </div>
          ) : (
            <div className="space-y-2">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                        ...
                        {article.hasAbstract && (
                          <span className="text-green-400">{article.abstractLength} chars</span>
                        )}
                      </div>
