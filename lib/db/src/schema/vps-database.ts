...
export const vpsDatabaseConfigTable = pgTable("vps_database_config", {
  ...
  sslCaCert: text("ssl_ca_cert").notNull().default(""),
  sslClientCert: text("ssl_client_cert").notNull().default(""),
  sslClientKey: text("ssl_client_key").notNull().default(""),
  sslRejectUnauthorized: boolean("ssl_reject_unauthorized").notNull().default(true),
